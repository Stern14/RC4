...
def len_and_slug(fields: dict):
    ...
    for key, value in fields.items():
        if len(value) != 2:
            return f"Неккоректное наименование {key}! \
Имя {key} должно быть длиной в 2(два) символа"

        try:
            validate_slug(value)
        except ValidationError:
            return f'Неккоректное наименование {key}! \
Имя {key} состоит только из латинских букв, \
цифр, символов подчеркивания или дефисов'
